...
theorem cast_descFactorial_eq_prod {n k : ℕ} :
  ...
  rw [descFactorial_eq_prod, prod_range_cast_nat_sub]
  ...
  have h₅ :
    0 <
      2 ^ f₂ k * γ ^ l * (1 - γ) ^ (redSteps γ k l ini).card *
        (beta γ k l ini / γ) ^ (densitySteps γ k l ini).card :=
    by positivity
...
theorem nine_three_part_two :
  ...
  have hγ₀' : 0 < γ := hγ₀.trans_le hγl
  have :
    exp (γ * (k - (redSteps γ k l ini).card)) ≤ (1 - γ) ^ (-k + (redSteps γ k l ini).card : ℝ) :=
    by
...
theorem nine_five_density :
  ...
  have hst :
    ((densitySteps γ k l ini).card : ℝ) ≤
      γ / (1 - γ) * (redSteps γ k l ini).card + 7 / (1 - 1 / 2) * k ^ (15 / 16 : ℝ) :=
    by
...
theorem nine_five :
  ...
  rw [tsub_add_eq_add_tsub htk, ← mul_assoc, ← mul_assoc, ← mul_assoc,
    rpow_neg (sub_pos_of_lt hγ₁).le (redSteps γ k l ini).card, div_pow, ← mul_assoc, rpow_nat_cast,
...
theorem edgeFinset_eq_filter [Fintype (Sym2 V)] (G : SimpleGraph V) [Fintype G.edgeSet]
    [DecidableRel G.Adj] : G.edgeFinset = univ.filter (· ∈ Sym2.fromRel G.symm) :=
  by
...
theorem edgeFinset_eq_filter_filter [DecidableEq V] [Fintype (Sym2 V)] (G : SimpleGraph V)
    ...
    G.edgeFinset = (univ.filter fun a : Sym2 V => ¬a.IsDiag).filter (· ∈ Sym2.fromRel G.symm) :=
  by
...
theorem sum_sym2 {α β : Type*} [DecidableEq α] [AddCommMonoid β] {s : Finset α} {f : Sym2 α → β} :
  ...
  have : (s.offDiag.filter fun z => ⟦z⟧ = ⟦(x, y)⟧) = ({(x, y), (y, x)} : Finset _) :=
    by
    ext ⟨x₁, y₁⟩
...
theorem sum_offDiag {α β : Type*} [DecidableEq α] [AddCommMonoid β] {s : Finset α}
    {f : α × α → β} : ∑ x in s.offDiag, f x = ∑ x in s, ∑ y in s.erase x, f (x, y) :=
  by
  rw [sum_sigma']
...
theorem sum_powersetCard_erase {α β : Type*} [Fintype α] [DecidableEq α] [AddCommMonoid β] {n : ℕ}
    ...
    ∑ U in powersetCard n s, ∑ y in Uᶜ, f U y = ∑ y, ∑ U in powersetCard n (s.erase y), f U y :=
  by
...
theorem powersetCard_filter_mem {α : Type*} [DecidableEq α] {n : ℕ} {s : Finset α} {x : α}
    ...
    ((powersetCard (n + 1) s).filter fun U => x ∈ U) =
      (powersetCard n (s.erase x)).image (insert x) :=
  by
...
theorem sum_powersetCard_insert {α β : Type*} [DecidableEq α] [AddCommMonoid β] {n : ℕ}
    ...
    ∑ U in powersetCard (n + 1) s, ∑ x in U, f U x =
      ∑ x in s, ∑ U in powersetCard n (s.erase x), f (insert x U) x :=
  by
  have :
    ∑ x in s, ∑ U in powersetCard n (s.erase x), f (insert x U) x =
      ∑ x in s, ∑ U in (powersetCard (n + 1) s).filter fun U => x ∈ U, f U x :=
    by
...
theorem sum_pair_subset {α β : Type*} [Fintype α] [DecidableEq α] [AddCommMonoid β] {n : ℕ}
    ...
    ∑ U in powersetCard (n + 1) s, ∑ x in U, ∑ y in Uᶜ, f U x y =
      ∑ x in s, ∑ y in univ.erase x, ∑ U in powersetCard n (s \ {x, y}), f (insert x U) x y :=
  by
...
theorem density_eq_average_partition [DecidableEq V] (G : SimpleGraph V) [DecidableRel G.Adj]
  ...
  have :
    ∑ U : Finset V in powersetCard n (univ \ {x, y}),
      ...
      ∑ U : Finset V in powersetCard n (univ \ {x, y}), 1 / ((n + 1) * (card V - (n + 1))) :=
    by
    refine sum_congr rfl fun U hU => _
...
theorem uLowerBoundRatio_lower_bound_aux_aux {k l m n : ℕ} {γ δ : ℝ} (hml : m ≤ l) (hk₀ : 0 < k)
  ...
  exact div_le_div_of_le (by norm_num1) (gamma_hMul_k_le_m_of hml hk₀ hg)
...
theorem maximally_good_clique {n k l : ℕ} {ξ ξ' : ℝ} {χ : TopEdgeLabelling (Fin n) (Fin 2)}
    ...
    1 - (1 + ξ') * ((l - x.card : ℝ) / (k + l - x.card)) ≤
      (χ.pullback (Function.Embedding.subtype (· ∈ commonBlues χ x))).density 0 :=
  by
  classical
  ...
  have hz :
    ∀ i ∈ commonBlues χ x,
      (((blue_neighbors χ) i ∩ commonBlues χ x).card : ℝ) <
        (commonBlues χ x).card * ((1 + ξ) * ((l - x.card) / (k + l - x.card))) :=
    by
    intro i hi
...
theorem nine_one_precise (γ₀ : ℝ) (hγ₀ : 0 < γ₀) :
  ...
  have hγ'₀ : 0 ≤ γ' := by
    rw [hγ'_eq]
    positivity
  have hxy' : ∀ (i) (_ : i ∉ x), IsGoodClique (1 / 16) k l χ (insert i x) → False :=
    by
    intro i hi hi'
